...
const NAV = [
  ...
  { label: 'Kamar Operasi', href: '/kamar-operasi', icon: 'M14.121 14.121L19 19m-7-7l7-7m-7 7l-2.879 2.879M12 12L9.121 9.121m0 5.758a3 3 0 10-4.243 4.243 3 3 0 004.243-4.243zm0 0L9.121 9.121' },
  ...
  { label: 'Gizi', href: '/gizi', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
  { section: 'Keuangan' },
  ...
  { label: 'Analitik', href: '/dashboard-analitik', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
  { section: 'Sistem' },
...
export function Sidebar() {
  ...
  return (
    <aside className={`flex flex-col glass-0 border-r transition-all duration-300 ${collapsed ? 'w-16' : 'w-[250px]'}`}
      style={{ padding: collapsed ? '24px 8px' : '24px 16px' }}>
      {/* Logo */}
      <div className={`flex items-center mb-6 ${collapsed ? 'justify-center' : 'gap-3 px-2.5'}`}>
        <div className="w-8 h-8 rounded-[10px] flex items-center justify-center flex-shrink-0"
          style={{ background: 'linear-gradient(135deg, var(--primary), var(--teal))' }}>
          <span className="text-[11px] font-extrabold text-white">PB</span>
        </div>
        {!collapsed && <span className="text-[14px] font-bold" style={{ color: 'var(--text-1)' }}>SIMRS</span>}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto space-y-0.5">
        {NAV.map((item, i) => {
          if ('section' in item) {
            ...
            return <p key={i} className="overline px-2.5 pt-5 pb-1.5">{item.section}</p>;
          }
          const active = pathname === item.href || pathname?.startsWith(item.href + '/');
          return (
            <Link key={item.href} href={item.href!}
              className={`group flex items-center gap-[9px] px-2.5 py-2 rounded-[10px] text-[13px] font-medium transition-all duration-150 ${
                active
                  ? 'text-[var(--primary-soft)]'
                  : 'text-[var(--text-2)] hover:text-[var(--text-1)] hover:bg-[rgba(255,255,255,0.08)]'
              }`}
              style={active ? { background: 'var(--primary-dim)' } : {}}
              title={collapsed ? item.label : undefined}>
              <svg className="w-[17px] h-[17px] flex-shrink-0" style={{ opacity: active ? 1 : 0.55 }}
      ...
      <button onClick={() => setCollapsed(!collapsed)}
        className="flex items-center justify-center py-2 rounded-[10px] text-[var(--text-3)] hover:text-[var(--text-2)] hover:bg-[rgba(255,255,255,0.04)] transition mt-4">
